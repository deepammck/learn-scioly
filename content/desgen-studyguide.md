...
## 1.10 Calico cats and X-linked coat color

Coat color in cats is X-linked, providing a vivid demonstration of X-inactivation. The two alleles at the *O* locus are **X^O** (orange/yellow, produces phaeomelanin) and **X^+** (black/brown, produces eumelanin).

**Genotype → phenotype:**

- **X^O X^O** (homozygous female): orange cat
- **X^+ X^+** (homozygous female): black cat
- **X^O X^+** (heterozygous female): **calico/tortoiseshell** — patches of orange AND black, because random X-inactivation means each cell expresses only one allele; clonal expansion of each cell produces a colored patch
- **X^O Y** (male): orange; **X^+ Y** (male): black
...
**Key tested fact:** calico/tortoiseshell coat in cats is direct visible evidence of X-inactivation. A calico cat is almost always female (or XXY). Under normal circumstances a calico cannot be male.

## 1.11 Nail-Patella Syndrome
...
**Inheritance pattern:** NPS is **autosomal dominant**, caused by heterozygous loss-of-function mutations in the **LMX1B** gene (a LIM-homeodomain transcription factor important in limb dorsoventral patterning). The disease mechanism is **haploinsufficiency** — one functional copy of LMX1B is insufficient for normal development, so a single mutated copy causes disease even though the other copy is intact. This distinguishes it from typical recessive loss-of-function disorders where both copies must be lost.
...
**Albinism as an epistasis example:** Albinism (loss of the enzyme **tyrosinase**, which is upstream in the melanin synthesis pathway) is the canonical example of recessive epistasis. An albino individual (genotype *cc* at the albinism locus) appears white/unpigmented **regardless of their genotype at other pigmentation loci** — the albinism gene is **epistatic** to other coat/eye/skin color genes. Importantly, albinism is also **pleiotropic**: the same single-gene mutation affects skin pigmentation, hair pigmentation, eye pigmentation, AND visual function (due to abnormal development of optic pathways that depend on melanin during eye development). When a question asks whether albinism is "epistatic" or "pleiotropic," both framings are correct but address different aspects — epistatic = it masks other pigmentation genes; pleiotropic = one gene affects multiple traits.
...
## 1.12 Bayesian / conditional probability for carrier risk

The basic "2/3 of unaffected offspring from Aa × Aa are carriers" logic can be extended by **Bayes’ theorem** when additional observations (e.g., multiple unaffected children) are available to update the probability.

**Framework:**

P(carrier | observations) = P(observations | carrier) × P(carrier) / P(observations)

**Classic problem structure:** A woman’s brother has an autosomal recessive condition (both parents obligate carriers Aa × Aa). The woman is unaffected — so she is either AA (prior prob 1/3) or Aa (prior prob 2/3). She marries an unaffected man and they have *k* unaffected children. What is the updated probability she is a carrier?

**Step-by-step:**

1. **Prior probabilities:** P(she is AA) = 1/3; P(she is Aa) = 2/3.
2. **Likelihood of *k* unaffected children given each hypothesis:** if husband is a known carrier (Aa × Aa), each unaffected child has P = 3/4; if husband is not a carrier, all children are unaffected regardless of her genotype (likelihoods equal → observations add no information in that case).
3. **Apply Bayes:** multiply prior × likelihood for each hypothesis, then normalize.

**Worked example (husband is also an obligate carrier; 2 unaffected children):**
- Posterior ∝ prior × likelihood: carrier: (2/3)(9/16) = 18/48; AA: (1/3)(1) = 16/48.
- P(carrier | 2 unaffected children, husband = carrier) = 18/(18+16) = **9/17 ≈ 0.53.**

**Simpler one-child case:** prior P(carrier) = 2/3. One unaffected child with carrier husband. P(unaffected | Aa × Aa) = 3/4; P(unaffected | AA × Aa) = 1.
- Posterior ∝ (2/3)(3/4) : (1/3)(1) = 1/2 : 1/3 → P(carrier) = (1/2)/(5/6) = **3/5; P(AA) = 2/5.**

**The universal method:** identify prior, compute likelihood of the observed data under each hypothesis, multiply, normalize. The key variable is whether the husband is a carrier — this changes the likelihoods dramatically.
...
### Cellular senescence vs. G0 vs. quiescence

These three non-dividing states are distinct:
...
## 2.8 Oogenesis and polar bodies

Meiosis is **asymmetric in females** — the cell does not divide cytoplasm equally. This is the critical contrast with spermatogenesis.
...
**Why unequal?** The asymmetric spindle attachment lets the oocyte retain nearly all cytoplasm, yolk, and organelles — essential for embryo development. The polar bodies are a disposal mechanism for the extra haploid nuclei.
...
## 2.9 Polyploid gamete probability

Polyploidy is complex to analyze quantitatively. For **autotetraploid (4n) individuals**, gamete genotype frequencies are calculated combinatorially:

- **AAAA** → only **AA** gametes
- **AAAa** → **AA : Aa = 1:1** (one A segregates with the a)
- **AAaa** → gametes **AA : Aa : aa = 1:4:1** (combinatorial from choosing 2 of 4 chromosomes: C(2,2)/C(4,2) = 1/6 AA; C(2,1)·C(2,1)/C(4,2) = 4/6 Aa; C(2,2)/C(4,2) = 1/6 aa)
- **Aaaa** → **Aa : aa = 1:1**

The key formula for **AAaa → gametes**: the 1:4:1 ratio arises because there are 6 equally likely ways to choose 2 chromosomes from 4: C(4,2) = 6.

## 2.10 Non-homologous chromosome associations during meiosis

Under normal meiotic conditions, **homologous chromosomes pair** via synapsis (synaptonemal complex) during prophase I — this is one of the most fundamental requirements of meiosis. Non-homologous pairing does not occur regularly.

Exceptions and contexts where non-standard pairing is relevant:

- **Robertsonian translocations** — two non-homologous acrocentric chromosomes fuse; during meiosis in a carrier, this creates a trivalent (three chromosomes associating) rather than normal bivalent pairing → abnormal segregation.
- **Ectopic pairing** — non-homologous chromosome regions containing repetitive sequences can occasionally associate, particularly in some insect (*Drosophila*) cytogenetics.
- **Errors leading to nondisjunction** — improper pairing/synapsis contributes to chromosomes segregating incorrectly.

Under normal conditions, the answer to "does non-homologous pairing occur regularly?" is **no** — homologous pairing is the rule; non-homologous associations are exceptions tied to structural chromosomal abnormalities or specific organisms.

## 2.11 Trisomy X (47,XXX) and dosage compensation

**Trisomy X** (47,XXX) is a sex chromosome aneuploidy with three X chromosomes.

**Dosage compensation:** as in typical 46,XX females, **X-inactivation** occurs. In 47,XXX individuals, **two of the three X chromosomes are inactivated** (forming two Barr bodies), leaving one active X — the same end state as a typical 46,XX cell. This is why trisomy X often produces a **mild or subclinical phenotype** — tall stature and sometimes mild learning differences are reported, but many individuals are never diagnosed.

**Molecular mechanism:** X-inactivation is mediated by the **XIST** non-coding RNA, which coats the inactive X chromosomes and recruits **histone-modifying complexes** — the inactive X(s) acquire heterochromatin-associated histone marks (e.g., H3K27 trimethylation via Polycomb repressive complex 2) and become condensed Barr bodies.

**Germline considerations:** in female germ cells, **X-reactivation occurs during oogenesis** (both/all X chromosomes become active again in oocytes prior to meiosis). In a 47,XXX individual, germ cells may transiently have **three active X chromosomes** during this reactivation window — this is consistent with **reduced fertility/earlier menopause** sometimes reported in trisomy X, though most individuals are fertile.

**Effect on general protein expression:** X-inactivation largely normalizes X-linked gene dosage toward typical (46,XX) levels, despite the extra chromosome — protein expression is approximately normal rather than 1.5× normal.

## 2.12 Chronic myelogenous leukemia and the Philadelphia chromosome

CML is the textbook example of a cancer caused by a specific chromosomal translocation.

**The Philadelphia chromosome** results from a **reciprocal translocation between chromosomes 9 and 22**, written **t(9;22)(q34;q11)**. This translocation fuses part of the **ABL1** gene (chromosome 9) with part of the **BCR** gene (chromosome 22), creating the **BCR-ABL1 fusion gene** on the shortened chromosome 22 (the "Philadelphia chromosome"). The BCR-ABL1 fusion protein is a **constitutively active tyrosine kinase**, driving uncontrolled proliferation of myeloid cells — the molecular basis of CML.

CML/Philadelphia chromosome is the canonical real-world example of a translocation with a clear phenotypic consequence. It was historically the first consistent chromosomal abnormality linked to a specific cancer.

## 2.13 Down syndrome features

Down syndrome (trisomy 21) features include:
- **Intellectual disability** (variable severity)
- Characteristic **facial features**: epicanthal folds, flattened nasal bridge, upslanting palpebral fissures, small ears
- **Hypotonia** (low muscle tone), especially in infancy
- **Congenital heart defects** (~40–50% of cases)
- Increased risk of **hypothyroidism**, **leukemia** (especially ALL and AML), and **early-onset Alzheimer's disease** (chromosome 21 carries the APP gene)
- **Brushfield spots** (light spots in the iris)

Note: **premature breast enlargement is not a feature of Down syndrome** — if offered as an answer choice among Down syndrome symptoms, it is the distractor/incorrect option.
...
## 2.14 AID mechanism detail

AID (activation-induced cytidine deaminase) drives both class-switch recombination and somatic hypermutation. Its biochemical mechanism:

**AID is a cytidine deaminase** — it converts **cytosine (C) → uracil (U)** in single-stranded DNA, creating a C→U mismatch. This initiating lesion is processed differently depending on context:
...
## 2.15 APOBEC3 catalytic domain

**APOBEC3** family proteins (cytidine deaminases involved in innate antiviral defense, and a major source of mutations in some cancers) characteristically have a **two-domain structure**: an **N-terminal domain (NTD)** and a **C-terminal domain (CTD)**.

For APOBEC3 enzymes with two deaminase domains (e.g., **APOBEC3G**), it is the **C-terminal domain (CTD) that contains the catalytically active deaminase site** responsible for cytidine-to-uridine deamination. The N-terminal domain in these proteins is often catalytically inactive but plays roles in substrate binding, packaging into viral particles, or oligomerization.

This is mechanistically related to AID (also an APOBEC family member): both use C→U deamination in ssDNA to generate mutations or DNA lesions.
...
## 3.10 Mutation-selection balance and heterozygote advantage equilibrium

These quantitative formulas give the *equilibrium frequency* of a deleterious allele when held up by mutation pressure or by heterozygote advantage.
...
**Heterozygote advantage (balancing selection) equilibrium** — when the heterozygote (A₁A₂) is fitter than both homozygotes, both alleles are maintained. Let w₁₁ = 1 − s₁ (fitness of A₁A₁), w₁₂ = 1 (heterozygote, fittest), w₂₂ = 1 − s₂ (fitness of A₂A₂). At equilibrium:
...
- **Sickle-cell example:** if s₁ = 0.2 (against HbA/HbA, susceptible to malaria) and s₂ = 0.8 (against HbS/HbS, sickle-cell disease), q̂ = 0.2/(0.2+0.8) = 0.20 (20% HbS allele maintained in a malarial environment).
...
## 3.11 Haldane mapping function and Kosambi mapping function

The simple 1 cM = 1% recombination conversion works well for short distances but breaks down at longer intervals, where **multiple crossovers** become invisible (double crossovers restore the parental combination and are counted as non-recombinant → you **undercount** crossovers). **Mapping functions** correct for this.
...
- At small distances: r ≈ d (functions converge — 1 cM ≈ 1% recombination holds under ~15 cM).
- At large distances: r approaches 0.5 (asymptote) while d can exceed 50 cM.
...
**Kosambi mapping function** (accounts for positive interference — nearby crossovers inhibit each other; more biologically realistic):

**r = ½ · tanh(2d)**; inverse: **d = ¼ ln[(1+2r)/(1−2r)]**

- Kosambi gives shorter map distances than Haldane for the same r (because it models the interference that reduces multiple crossovers).
...
## 3.12 BLOSUM scoring matrices

**BLOSUM (BLOcks SUbstitution Matrix)** matrices are used in protein sequence alignment (e.g., BLAST) to score how "costly" or "favorable" it is to substitute one amino acid for another, based on observed substitution frequencies in real protein families.

**General principle:** BLOSUM matrices assign **positive scores** to substitutions that occur frequently between related proteins (conservative substitutions, e.g., Leu↔Ile — similar chemistry) and **negative scores (penalties)** to substitutions that occur rarely (radical substitutions between chemically dissimilar amino acids).

**Lowest-scoring (most negative) substitutions** in matrices like BLOSUM62 tend to involve **tryptophan (W)** paired with small or charged residues (e.g., W↔D) — tryptophan is large, hydrophobic, and structurally constrained, so substituting it for almost anything is rarely tolerated and rarely observed in nature.

**Common BLOSUM variants:** BLOSUM62 (built from sequences sharing ~62% identity) is the most widely used for general-purpose alignment; BLOSUM45 is used for more divergent sequences; BLOSUM80 for closely related sequences.

**Conceptual takeaway:** a BLOSUM substitution score reflects evolutionary tolerance — frequently observed substitutions in real proteins get positive scores; rare substitutions get negative ones. Structurally/chemically distinctive residues (Cys, Trp) carry the most negative penalties when substituted.
...
## 4.7 G-quadruplexes (G4 structures)

A **G-quadruplex (G4)** is a non-B-DNA secondary structure formed by **G-rich sequences** — particularly in **telomeric DNA** (TTAGGG repeats) and gene promoters.

**Structure:** Four guanines hydrogen-bond in a planar **G-quartet** (square arrangement); multiple G-quartets stack to form a G4. G-quartets are held by **Hoogsteen hydrogen bonds** (N7 and O6 of guanine bond to the donor/acceptor on the partner) — distinct from Watson-Crick base pairing (N1, N2 face).

**Structure types:** G4s can form from one strand folded back on itself (intramolecular) or from multiple strands. Topology varies (parallel, antiparallel, mixed). Diagonal loops connect G-runs that fold across the top or bottom of the G-quartet stack.

**Biological significance:**

- **Telomeres:** the 3′ G-rich overhang can fold into G4 structures; **telomerase must unfold these** to extend the telomere. G4 stabilization by small-molecule ligands = an anti-cancer strategy to block telomerase.
...
- **C9orf72 repeat expansion:** the GGGGCC hexanucleotide repeat in C9orf72 (a cause of ALS and FTD) forms G4 structures, contributing to RNA foci formation and impaired gene expression (see §4.9 for details).
...
## 4.8 Amphipathic molecules (membrane context)
...
**Classic examples:** Phospholipids (polar phosphate head + nonpolar fatty acid tails) are amphipathic — this is why they spontaneously form bilayers in water. Detergents/soaps follow the same logic. Many membrane proteins contain amphipathic helices at the membrane-water interface.

**Relevance to DNA/cell biology:** the amphipathic nature of phospholipids is what enables lipid nanoparticle (LNP) delivery systems for mRNA (§6.11); the ionizable lipid component must balance hydrophilic/hydrophobic properties for both membrane fusion and aqueous stability.
...
**Number of origins of replication in a human cell:** the human genome is ~3.2 × 10⁹ bp. Eukaryotic replicons (the stretch replicated from one origin) are typically tens to a few hundred kilobases. Dividing genome size by typical replicon size gives **~10⁴–10⁵** origins. The standard order-of-magnitude estimate for total replication origins across the human genome in a single S phase is **~10⁵** (some sources cite 30,000–50,000 "active" origins per S phase; licensed-but-dormant origins push the total higher). For a multiple-choice order-of-magnitude question, **10⁵** is the conventional answer.
...
## 4.9 Extended telomere functions

The guide describes telomeres as buffers against replication-end loss. The full tested set of functions:

1. **Buffer against replication-end loss:** each replication round shortens the chromosome; telomeric TTAGGG repeats are expendable buffer, protecting coding sequence.
2. **Prevent end-to-end chromosome fusion:** without telomeres, the NHEJ machinery would treat chromosome ends as double-strand breaks and fuse them → dicentric chromosomes → breakage-fusion-bridge cycles.
3. **Prevent activation of the DNA damage response (DDR):** the protective protein complex **shelterin** (TRF1, TRF2, POT1, etc.) caps the telomere, preventing ATM/ATR kinases from recognizing the end as a DSB. Loss of shelterin or critically short telomeres → DDR activation → senescence or apoptosis.
4. **Chromosome anchoring/nuclear organization:** telomeres associate with the nuclear lamina and help organize chromosome positioning in the nucleus.

**Telomerase:** a reverse transcriptase (protein + RNA template, **TERC**) that extends telomeres in germ cells, stem cells, and most cancers. The RNA template carries 3′-AAUCCC-5′, which templates addition of TTAGGG repeats. **Most somatic cells lack telomerase** → progressive shortening → eventual senescence/apoptosis limit (**Hayflick limit**, ~50 divisions). Cancer cells reactivate telomerase (or use **ALT** — Alternative Lengthening of Telomeres via HR) to achieve replicative immortality.

## 4.10 Frequency of structural genes in the human genome

"Structural genes" refers to genes encoding proteins or functional RNAs (as opposed to regulatory sequences, introns, or repetitive non-coding DNA).

**Human genome:** protein-coding exons represent roughly **1–2% of the genome** (~20,000 protein-coding genes). The rest is introns, regulatory elements, repetitive sequences, and DNA of largely unclear function.

**Bacterial genomes:** far more gene-dense — protein-coding sequence accounts for **~85–90%+** of the genome, with minimal intergenic DNA.

## 4.11 Sequence-independent dsDNA binding

"Sequence-independent dsDNA binding" describes proteins that bind double-stranded DNA based on its **general structural features** (the double helix, backbone geometry, minor/major groove) rather than recognizing a specific base sequence.
...
- **Architectural/structural DNA-binding proteins in bacteria** (e.g., **HU, H-NS**) — bind and bend DNA without strong sequence specificity; important for nucleoid organization (analogous to histones).
- **Topoisomerases** — act on DNA topology (supercoiling) rather than specific sequences, though some prefer certain sites.

**Contrast with sequence-dependent binding:** restriction enzymes, transcription factors, Cas9 (PAM-dependent) all recognize specific sequences. Histones and architectural proteins recognize the backbone/structure — the distinction is often the test point.
...
## 4.12 Holliday junctions and homologous recombination intermediates

The guide covers HR as a DSB repair mechanism but not the molecular intermediates. The **Holliday junction** is the central intermediate.

**Key steps in prokaryotic HR (RecBCD pathway, E. coli):**
...
2. At χ, RecBCD switches from degrading to generating a **3′ single-stranded tail**; **RecA** (the bacterial Rad51 equivalent) coats the ssDNA and catalyzes **strand invasion** into the homologous sequence.
3. **Holliday junction (HJ):** the four-way DNA intermediate formed when two homologous duplexes are joined by strand exchange — a cross-shaped structure where two strands have been exchanged between the duplexes.
4. **Branch migration:** the HJ moves along the DNA (catalyzed by **RuvAB** helicase), extending the region of heteroduplex.
5. **Resolution:** **RuvC** (an endonuclease) recognizes and cleaves the HJ in two orientations → either patch (non-crossover) or splice (crossover) products.

**Eukaryotic equivalent:** Rad51 (RecA homolog) for strand invasion; Mus81/Eme1 or GEN1 for resolution. The intermediate logic is the same.

**Why this matters:** HR always goes through a Holliday junction (or resolves via SDSA — synthesis-dependent strand annealing). NHEJ does not. The HJ is the reason HR requires a template and is accurate; NHEJ skips it and is error-prone.
...
## 4.13 C9orf72 hexanucleotide expansion

C9orf72 repeat expansions (GGGGCC, located in intron 1) are the most common genetic cause of ALS and frontotemporal dementia. Normal individuals carry 2–10 hexanucleotide GGGGCC repeats; more than a few hundred repeats are pathogenic.

**Three non-mutually-exclusive toxicity mechanisms:**

1. **Loss of function / haploinsufficiency:** the expanded allele is transcribed less (due to hypermethylation), reducing normal C9orf72 protein levels.
2. **RNA toxic gain-of-function:** the expanded repeats are transcribed bidirectionally and form RNA foci in neurons, sequestering RNA-binding proteins and impairing RNA processing. The GGGGCC repeats form **G-quadruplex structures** (see §4.7), which significantly impair expression and function of C9orf72.
3. **Toxic dipeptide-repeat (DPR) protein gain-of-function:** non-ATG (RAN — repeat-associated non-AUG) translation of the hexanucleotide repeat expansions produces dipeptide-repeat proteins, whose accumulation is toxic to neurons.

Additional downstream consequences include **nucleocytoplasmic transport defects** (DPR proteins and RNA foci disrupt the nuclear pore complex) and **mitochondrial dysfunction**.
...
## 5.6 m6A (N6-methyladenosine) RNA modification

**m6A** is the most abundant internal chemical modification on eukaryotic mRNA — methylation at the N6 position of adenosine.

**Timing:** m6A is deposited **co-transcriptionally**, while the nascent transcript is still being synthesized by RNA Pol II and often still associated with chromatin. The "writer" complex (core catalytic subunit **METTL3**, paired with **METTL14** and accessory proteins like WTAP) methylates adenosines, predominantly within a **DRACH consensus motif**, often enriched near stop codons and in 3′ UTRs.

**Downstream effects:** "reader" proteins (YTHDF1/2/3, YTHDC1/2) recognize m6A marks and influence mRNA splicing, nuclear export, stability/decay, and translation efficiency. "Eraser" enzymes (FTO, ALKBH5) can remove the mark, making it a reversible/dynamic modification — analogous in concept to histone marks but on RNA ("epitranscriptomics").

**If asked "before, during, or after transcription":** the best-supported answer is **during (co-transcriptional)**, though some m6A deposition can also occur post-transcriptionally on already-exported mRNA.
...
1. **Initiation:** small (30S) subunit binds mRNA at the **Shine–Dalgarno sequence** (ribosome-binding site upstream of AUG, base-pairs with 16S rRNA); initiator **fMet-tRNA** binds the start codon in the P site; large (50S) subunit joins. Initiation factors (IF1–3) assist — their individual roles:
   ...
   - **IF2 (a GTPase):** binds **fMet-tRNA** and escorts it to the P site of the 30S ribosome; GTP hydrolysis upon 50S joining releases IF2.
...
## 5.7 Protein termini, folding, and Levinthal's paradox

**N- vs. C-terminus:**

- Translation proceeds such that the **first amino acid incorporated becomes the N-terminus** (retains a free amino group); synthesis proceeds toward the **C-terminus** (last amino acid added, free carboxyl group).
- This mirrors mRNA's 5′→3′ reading direction: **5′ end of mRNA → N-terminus of protein; 3′ end of mRNA → C-terminus of protein.**
- During protein folding, **hydrophobic side chains cluster toward the interior** of the folded protein (hydrophobic effect, entropy-driven), while hydrophilic/charged side chains tend to remain on the surface in contact with water.

**Levinthal's paradox and the combinatorial explosion of conformations:**

A 100-residue polypeptide with ~2 backbone dihedral angles (φ and ψ) per residue, each with ~3 possible values, could theoretically adopt ~3^(2×99) = **3^198** conformations. This number is astronomically large (~10^94). Even sampling 10^13 conformations per second, exploring all of them would take longer than the age of the universe — yet real proteins fold in milliseconds to seconds.

**Resolution:** proteins do not randomly search all conformations. Folding follows defined **energetic funnels** (pathways) that bias the search, with chaperones assisting. Levinthal's paradox illustrates why folding mechanisms must be non-random; it does not imply that proteins cannot fold, only that they cannot fold by random search.
...
## 6.7 Pfu vs. Taq polymerase — fidelity

Both are thermostable polymerases used in PCR, but they differ in a key enzymatic property:

- **Taq polymerase** (from *Thermus aquaticus*) **lacks 3′→5′ exonuclease (proofreading) activity.** Fast and robust but relatively high error rate (~1 error per 10⁴–10⁵ nucleotides); adds non-templated 3′ A overhangs (useful for TA cloning but a source of error).
- **Pfu polymerase** (from *Pyrococcus furiosus*) **has 3′→5′ proofreading exonuclease activity**, giving ~10–100× higher fidelity than Taq. Tradeoff: Pfu is **slower** (lower processivity/extension rate) and produces **blunt ends**.

**For “what is the advantage of Pfu over Taq”:** the answer is **proofreading / higher fidelity (lower error rate)** due to 3′→5′ exonuclease activity — the same logic as Pol III proofreading in replication (§4.3), applied to PCR enzymes.
...
**Overlapping peaks in Sanger = heterozygosity:** when a sequenced sample is **heterozygous at a position**, both alleles are present in the PCR product → at the heterozygous position, two different ddNTP terminations occur simultaneously → **two overlapping fluorescent signals at the same position.** The basecalling software may call an “N” or show a double peak (e.g., two overlapping colors). In clinical sequencing, overlapping peaks are the standard indicator of a heterozygous SNP or mutation.
...
**CRISPR RNA components (tested at CARSO):**

- **crRNA (CRISPR RNA):** the processed spacer RNA containing the ~20 nt guide sequence complementary to the target DNA. Derived from the CRISPR array in the bacterial genome.
- **tracrRNA (trans-activating crRNA):** a structural RNA that base-pairs with the crRNA repeat region and recruits Cas9.
- **sgRNA (single guide RNA):** an engineered fusion of crRNA + tracrRNA in one molecule — what is used in most CRISPR experiments (simpler to deliver).
- **PAM (protospacer adjacent motif):** a short sequence (NGG for SpCas9) immediately 3′ of the target on the non-template strand. Cas9 requires it for binding and cleavage — provides self vs. non-self discrimination in bacteria.

**Cas9 is an endonuclease, not an exonuclease:** Cas9 makes a double-strand break *within* (internal to) a DNA molecule — the defining property of an endonuclease. It does not chew nucleotides off DNA ends (that would be exonuclease activity, which Cas9 lacks). Its two nuclease domains — **RuvC** (cuts the non-target strand) and **HNH** (cuts the target strand) — each make a single internal cut, producing a blunt DSB ~3 bp upstream of the PAM.

**Cas9 metal ion cofactor:** both the RuvC and HNH active sites require **Mg²⁺** (magnesium) for catalysis — a two-metal-ion mechanism typical of many nucleases and polymerases. In the absence of Mg²⁺, Cas9 cannot cleave DNA. (In vitro, Mn²⁺ can sometimes substitute, often with altered specificity.)

**Cas9 domain mutations — nickase vs. dCas9:**

- A single point mutation in **either** nuclease domain — classically **D10A** (inactivates RuvC) or **H840A** (inactivates HNH) — produces a **nickase (nCas9)**, which can make a nick in only one strand (partial activity).
- **Both mutations together (D10A + H840A)** produce **dead Cas9 (dCas9)** — all catalytic activity abolished, but DNA binding via guide RNA is retained. dCas9 fused to activators/repressors is used for **CRISPRa/CRISPRi** without cutting.
- One domain mutation alone does NOT abolish all activity — that is the conceptual trap often tested.

**Cas12a (Cpf1) structure — REC1/REC2 domains:**

Cas12a is a Class 2, Type V CRISPR effector used for genome editing (e.g., DETECTR diagnostics). Its architecture:

- A **recognition (REC) lobe** made of two α-helical domains, **REC1 and REC2**, which bind the crRNA–target DNA heteroduplex. REC2 undergoes large conformational shifts upon DNA binding that activate the nuclease.
- A **nuclease (NUC) lobe** containing the **RuvC** catalytic domain (Cas12a uses only one nuclease domain, unlike Cas9's RuvC+HNH), the **PAM-interacting (PI)** domain, and the **Wedge (WED)** domain.

Key contrasts with Cas9: Cas12a (1) uses a single RuvC domain to cleave both strands sequentially → **staggered ("sticky") cuts with 5′ overhangs** (Cas9 makes blunt cuts); (2) requires **crRNA only** (no tracrRNA/sgRNA); (3) recognizes a **T-rich PAM (5′-TTTV-3′) on the 5′ side** of the target (Cas9 uses 3′-NGG); (4) has **collateral ssDNA-cleavage ("trans") activity** once activated — the basis for DETECTR diagnostics.

**CRISPR system classification (Types I–IV):**

- **Class 1 (Types I, III, IV):** use **multi-subunit effector complexes** — multiple different Cas proteins assemble to find and cleave the target.
  - **Type I:** uses the multi-protein **Cascade complex** for target recognition + the helicase-nuclease **Cas3** for DNA degradation.
  - **Type III:** uses the **Csm/Cmr complex** (signature gene: **cas10**); can target both DNA and RNA; some subtypes encode reverse transcriptase.
  - **Type IV:** encodes a minimal multisubunit crRNA-effector complex; **lacks cas1 and cas2** (no spacer-acquisition/adaptation machinery) — functionally enigmatic; may rely on other systems for spacer acquisition.
- **Class 2 (Types II, V, VI):** use a **single large multidomain protein** as the effector (Cas9 = Type II; Cas12a/Cpf1 = Type V; Cas13 = Type VI). The simplicity of needing one protein + one guide RNA is why Class 2 systems became the dominant genome-editing tools.

**Type II CRISPR-Cas subtypes:** Type II is divided into subtypes II-A, II-B, and II-C. **Subtype II-A** is characterized by the accessory gene **csn2** (involved in spacer acquisition alongside Cas1-Cas2); example organism: *Streptococcus thermophilus*. II-B typically has **cas4** instead; II-C uses a more minimal adaptation module.

**Protospacer integration — repeat duplication:** during the CRISPR adaptation step, the **Cas1-Cas2** complex integrates a new spacer (captured foreign DNA) at the **leader-proximal end** of the CRISPR array. The integration reaction **duplicates the adjacent repeat** — the new spacer ends up flanked by repeats on both sides (one original, one duplicated). This is why CRISPR arrays maintain their alternating repeat-spacer architecture as they grow, and why the **leader-proximal spacers are the most recently acquired** (useful for reconstructing a population's immunological history against phages).

**Antisense/exon-skipping therapy:** Antisense oligonucleotides (ASOs) can be designed to cause the splicing machinery to skip a specific exon, restoring the reading frame in **out-of-frame deletions**. Classic example: **Duchenne muscular dystrophy (DMD)** — certain deletions shift the reading frame; skipping an adjacent exon restores a multiple-of-three reading frame, producing a shorter but partially functional protein (the basis of drugs like eteplirsen). Key limitation: only works for out-of-frame deletions where adjacent exon skipping can restore frame; does not help in-frame deletions or point mutations.

**Replica plating (tested at HUSO):** a technique to non-destructively test many colonies for a phenotype simultaneously.

1. Grow colonies on a **master plate** (permissive medium, no selection).
2. Press a **sterile velvet cloth** onto the master plate — velvet picks up cells from each colony in their exact spatial configuration.
3. Press the velvet onto one or more **replica plates** (different selective media, e.g., + antibiotic).
4. Colonies that grow on the replica plate are positive for that trait; their position maps back to the master plate to retrieve the colony.

Applications: identifying auxotrophic mutants, antibiotic-sensitive mutants. Historically important in the Lederberg experiment proving that mutations arise **randomly before selection** (colonies resistant to phage appeared in identical positions on unexposed and phage-exposed plates → resistance pre-existed exposure, not induced by it).

**LNP mRNA delivery — endosomal escape mechanism:**

Lipid nanoparticles (LNPs) are the delivery vehicle for mRNA vaccines and RNA therapeutics. A typical LNP contains: an **ionizable cationic lipid**, **cholesterol**, a **helper phospholipid**, and a **PEGylated lipid**.

1. LNP is taken up by the cell via **endocytosis**, ending up inside an **endosome**.
2. As the endosome matures, its interior becomes increasingly **acidic** (pH ~5–6).
3. The **ionizable lipid**, neutral at physiological pH, becomes **protonated (positively charged)** in the acidic endosome.
4. This charge change drives interaction with the negatively charged endosomal membrane phospholipids, **destabilizing/disrupting the endosomal membrane**.
5. The mRNA cargo escapes into the **cytoplasm** for translation. This **endosomal escape** step is the major bottleneck — most LNPs that enter cells fail to release cargo.

**Gene therapy classification (appears in practice set questions):**

Gene therapy can be classified by several frameworks:

**Somatic vs. germline:** Somatic gene therapy modifies only the treated individual's non-reproductive cells; effects are not inherited. Germline gene therapy would be heritable. Germline human gene therapy is prohibited or heavily restricted in most jurisdictions.

**In vivo vs. ex vivo:** In vivo delivers the therapeutic gene directly into the patient (e.g., AAV injected into the eye). Ex vivo removes cells from the patient, modifies them in the lab, and reinfuses them (e.g., CAR-T cell therapy).

**Mechanism:** Gene addition (adding a functional copy of a defective gene), gene editing (direct genomic correction via CRISPR or other nucleases), or gene silencing/inhibition (antisense oligonucleotides, RNAi/siRNA to knock down harmful gene expression).

**Vector classification:** Viral vectors (lentiviral/retroviral, adenoviral, AAV) vs. non-viral methods (lipid nanoparticles/LNPs, naked DNA, electroporation).
...
**Microarray probe design — oligonucleotide length:**
...
- **Short oligo arrays (Affymetrix-style):** probes are typically **~25 nucleotides** long. Short probes allow cheap, high-density synthesis (millions of features per chip), but any single 25-mer has weaker, less specific binding — multiple probes per transcript are used and results computationally combined.
- **Long oligo / spotted arrays:** probes are typically **50–70 nucleotides**, sometimes up to ~80–100 nt. Longer probes bind more specifically, tolerating single mismatches better, but fewer can fit per chip.

**Key tradeoff:** probe length balances **specificity** (longer = better discrimination of similar sequences, fewer cross-hybridization false positives) against **density/cost** (shorter = more features per chip, cheaper synthesis). For a question asking "why are probes short" or "why are probes long," frame the answer around this tradeoff. The conventional testable default is ~25 nt (Affymetrix); anything in the 25–70 nt range is plausible depending on the array type.
...
**Beta-strand topology:**

- Beta-strands in a beta-sheet can be arranged **parallel** (all strands running the same N→C direction) or **antiparallel** (adjacent strands running in opposite N→C directions). Antiparallel sheets have straighter, stronger hydrogen bonds and are generally more stable.
- Beta-strands in topology diagrams are depicted as **flat arrows**, with the arrowhead pointing toward the **C-terminal direction** of that strand. Parallel vs. antiparallel is determined by whether neighboring arrows point the same or opposite directions.
- "Relative positioning" questions ask you to identify which strands are adjacent/hydrogen-bonded based on a topology diagram, or to determine parallel vs. antiparallel arrangement from arrow directions.

**BLOSUM scoring — most negative penalties:** BLOSUM matrices (e.g., BLOSUM62) assign **positive scores** to amino acid substitutions that are frequently observed between related proteins (conservative, e.g., Leu↔Ile) and **negative scores** to rare, radical substitutions. The **most negative (highest-penalty) substitutions** tend to involve **tryptophan (W)** paired with small or chemically opposite residues (e.g., W↔D), because Trp is large, hydrophobic, and structurally distinctive — substituting it is rarely tolerated. The exact lowest value depends on the specific BLOSUM variant (BLOSUM45/62/80) and residue pair.
